const d3ArtFoldOutBox = document.getElementById("d3ArtFoldOutBox").getBoundingClientRect();
let w = d3ArtFoldOutBox.width;
let h = d3ArtFoldOutBox.height;
...
var tri1TransitionData = [
  {'x':center, 'y':hyp},
  {'x':center + (hyp/2)*Math.sqrt(3), 'y': center - (hyp/2)},
  {'x':center + (hyp/2)*Math.sqrt(3), 'y':center - (hyp * 3/2)}
]
...
var tri2TransitionData = [
  {'x':center + (hyp/2)*Math.sqrt(3), 'y': center - (hyp/2)},
  {'x':center + (hyp/2)*Math.sqrt(3), 'y': center + (hyp/2)},
  {'x':center + (hyp/2)*Math.sqrt(3) * 2, 'y':center}
]
...
let svg = d3.select("#d3ArtFoldOutBox")
  .append("svg")
  .attr("width", w)
  .attr("height", h)
